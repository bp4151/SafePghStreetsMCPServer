...
# @mcp.tool()
# def match_crashes_from_gpx(gpx_content: str, limit: int = 50) -> str:
#     """
#     Match WPRDC crash data with a GPX route by searching for bicycle crashes within the route's bounding box.
#     Uses cached data from get_wprdc_crash_data.
#
#     Args:
...
#         gpx = gpxpy.parse(gpx_content)
#     except Exception as e:
#         logger.error(f"Error parsing GPX content in match_crashes_from_gpx: {e}", exc_info=True)
#         return f"Failed to parse GPX content: {e}"
#
#     # Get bounding box of the GPX route
...
#         for segment in track.segments:
#             for point in segment.points:
#                 if min_lat is None or point.latitude < min_lat: min_lat = point.latitude
#                 if max_lat is None or point.latitude > max_lat: max_lat = point.latitude
#                 if min_lon is None or point.longitude < min_lon: min_lon = point.longitude
#                 if max_lon is None or point.longitude > max_lon: max_lon = point.longitude
#
#     # Check waypoints
#     for waypoint in gpx.waypoints:
#         if min_lat is None or waypoint.latitude < min_lat: min_lat = waypoint.latitude
#         if max_lat is None or waypoint.latitude > max_lat: max_lat = waypoint.latitude
#         if min_lon is None or waypoint.longitude < min_lon: min_lon = waypoint.longitude
#         if max_lon is None or waypoint.longitude > max_lon: max_lon = waypoint.longitude
#
#     if min_lat is None:
#         return "No coordinates found in the provided GPX content."
#
#     try:
#         # Get data from cache (or fetch if not cached)
#         all_records = _fetch_all_crash_data()
#
#         # Filter locally for coordinates (bicycle filter is now handled at the API level)
#         matched_records = []
#         for rec in all_records:
#             try:
#                 lat = float(rec.get("DEC_LAT"))
#                 lon = float(rec.get("DEC_LONG"))
#
...
#             return "No bicycle crash records found within the GPX route's bounding box."
#
#         output = [f"Found {len(matched_records)} bicycle crash records within the route area:"]
...
#         return "\n".join(output)
#     except Exception as e:
#         logger.error(f"Error in match_crashes_from_gpx: {e}", exc_info=True)
#         return str(e)
